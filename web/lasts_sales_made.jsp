...
<html>
    <head>
        ...
        <script>
            function impinvc(clt) {
                var url = "/SGRB/Receipt.jsp?clt=" + clt + "&user=Duplicate&sector=<%=s%>";
    ...
    <body>
        ...
        %>
        <h1><%=lang.lng(current_lang, "last_sales_made_in_descending_order")%></h1><br>
        ...
        <%          
            ...
            try {
                ...
                String qry = "select CONCAT(CONCAT(s.client_nr,' '), c.clientname) as msname_us_time,s.article as article,s.quantity as quantity,"
                        + "s.price as price,s.p_code as p_code,s.barcode as barcode,"
                        + "CONCAT(CAST(s.user_datetime AS DATE),CONCAT(' ',CAST(s.user_datetime AS TIME))) as user_datetime,"
                        + "(select username from users u where u.id = s.iduser limit 1) as user from sales s "
                        + " LEFT JOIN clients c ON s.client_nr = c.client_nr where s.sector = '" + s + "'"
                        + " order by s.user_datetime desc,s.id asc limit 1000";
